...
const EFFECTIVE_LIST_ITEM_LIMIT = Number.isInteger(LIST_ITEM_LIMIT) && LIST_ITEM_LIMIT > 0
  ? LIST_ITEM_LIMIT
  : 300000;
 ...
 * @returns {Promise<Object>}
 */
const getZeroTrustListItems = (id, page = 1) =>
  requestGateway(
    `/lists/${id}/items?per_page=${LIST_ITEM_PAGE_SIZE}&page=${page}`,
    {
    method: "GET",
    }
  );
...
/**
 * Gets all Zero Trust list items by traversing paginated API responses.
 * @param {string} id The id of the list.
 * @returns {Promise<Object[]>}
 */
const getAllZeroTrustListItems = async (id) => {
  let page = 1;
  let totalCount = Infinity;
  const allItems = [];

  while (allItems.length < totalCount) {
    const { result: listItems, result_info } = await getZeroTrustListItems(id, page);
    const items = listItems || [];
    totalCount = result_info?.total_count ?? items.length;
...
/**
 * Patches an existing list in batches to stay within the API's per-request item limit.
 * Removals are sent with the first batch; appends are chunked across as many requests as needed.
 * @param {string} listId
 * @param {{ remove?: string[], append?: Object[] }} patch
 */
const patchExistingListInBatches = async (listId, patch) => {
  const appends = patch.append || [];
  const removes = patch.remove || [];

  const appendChunks = [];
  for (let i = 0; i < appends.length; i += LIST_ITEM_PAGE_SIZE) {
    appendChunks.push(appends.slice(i, i + LIST_ITEM_PAGE_SIZE));
  }
  if (!appendChunks.length) appendChunks.push([]);

  for (let i = 0; i < appendChunks.length; i++) {
    const batchPatch = {};
    // Send all removals with the first batch only
    if (i === 0 && removes.length) batchPatch.remove = removes;
    if (appendChunks[i].length) batchPatch.append = appendChunks[i];
    if (Object.keys(batchPatch).length) {
      await patchExistingList(listId, batchPatch);
    }
  }
};
...
export const synchronizeZeroTrustLists = async (items) => {
  ...
  for (const list of cgpsLists) {
    const listItems = await getAllZeroTrustListItems(list.id);
  ...
  const patches = Object.fromEntries(
    Object.entries(removalPatches).map(([listId, patch]) => {
      ...
      const spaceInList = EFFECTIVE_LIST_ITEM_LIMIT - (domainsByList[listId].length - patch.remove.length);
  ...
  if (toAdd.length) {
    ...
    unpatchedListIds.forEach(listId => {
      const spaceInList = EFFECTIVE_LIST_ITEM_LIMIT - domainsByList[listId].length;
      if (spaceInList > 0) {
        // Take upto spaceInList entries from the additions into this list.
        const append = Array(spaceInList)
          .fill(0)
  ...
  for(const [listId, patch] of Object.entries(patches)) {
    ...
    await patchExistingListInBatches(listId, patch);
  }
...
export const defragmentZeroTrustLists = async () => {
  ...
  const assignedEntries = allEntries.map((entry, index) => {
    const listIndex = Math.floor(index / EFFECTIVE_LIST_ITEM_LIMIT);
...
export const createZeroTrustListsOneByOne = async (items, startingListNumber = 1) => {
  let totalListNumber = Math.ceil(items.length / EFFECTIVE_LIST_ITEM_LIMIT);

  for (let i = 0, listNumber = startingListNumber; i < items.length; i += EFFECTIVE_LIST_ITEM_LIMIT) {
    // We use the description as the list addition time because the API does not allow setting the created_at time.
    const chunk = items
      .slice(i, i + EFFECTIVE_LIST_ITEM_LIMIT)
