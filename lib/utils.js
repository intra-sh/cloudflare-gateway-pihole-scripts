    ...
    try {
      ...
      if (!response.ok) {
        ...
        const httpError = new Error(
          `HTTP error! Status: ${response.status}${responseBody ? ` - ${responseBody}` : ""}`
        );
      ...
      // Auth errors (401/403) are not transient — retrying will not help.
      if (status === 401 || status === 403) {
        throw new Error(
          `Authentication error (HTTP ${status}). ` +
          `Verify that CLOUDFLARE_API_TOKEN and CLOUDFLARE_ACCOUNT_ID are correctly set and that the token has the required Zero Trust Gateway permissions.`
        );
      }

      // Validation/request-shape errors are not transient.
      if (status === 400) {
        throw error;
      }
      ...
      if (status === RATE_LIMITING_HTTP_ERROR_CODE) {
