...
// Validate required credentials early so the script fails fast with a clear message
// rather than hitting 50 retried auth errors from the API.
// Skip in DRY_RUN mode since no API calls are made.
const isDryRun = !!parseInt(process.env.DRY_RUN, 10);
if (!isDryRun) {
  if (!API_TOKEN && !API_KEY) {
    console.error(
      "Error: CLOUDFLARE_API_TOKEN is not set. " +
      "Create an API token with Zero Trust Gateway Lists and Rules permissions and set it as an environment variable or GitHub Actions secret."
    );
    process.exit(1);
  }
  if (!ACCOUNT_ID) {
    console.error(
      ...
      "Find your Account ID on the Cloudflare dashboard and set it as an environment variable or GitHub Actions secret."
    );
    process.exit(1);
  }
}

const parsedListItemLimit = Number.parseInt(
  (process.env.CLOUDFLARE_LIST_ITEM_LIMIT || "")
    .trim()
    .replace(/[\s,_']/g, ""),
  10
);

export const LIST_ITEM_LIMIT = Number.isInteger(parsedListItemLimit) && parsedListItemLimit > 0
  ? parsedListItemLimit
  : 300000;

export const LIST_ITEM_PAGE_SIZE = 1000;
